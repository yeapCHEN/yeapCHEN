...
/*1.由于在设置PWM时将TIM_Pulse默认设置为1000，因此在初始化定时器2时，TIM_Period的
	值不能小于该值，可自行修改。TIM_Pulse的值并不会影响采样频率
  2.采样频率=定时器2溢出频率=SYSCLK/预分频值、溢出值。因此如果将TIM_Pulse设为1 ，
  	TIM_Period设为2，TIM_Perscaler设为1，理论上采样频率最高可达36Mhz 
*/ 
